      ...
      <v-card>
         ...
         <div class="explore">
            <div class="explore__body">
               <div class="explore__main--title"></div>
            </div>
         </div>
               ...
               <v-card flat>
                  <v-card-text>
                     <v-row>
                           ...
                           <div class="liked__card">
                              ...
                              <div class="liked__card-controls">
                                 <button
                                    ...
                                    @click="changePrice(item.item_id)"
                                 >
                                    Change price
                                 </button>
                                 <button
                        ...
                        <!-- MODAL CHANGE PRICE-->
                        <v-dialog v-model="priceModal" max-width="400" dark>
                           <v-card>
               ...
               <v-card flat>
                  <v-card-text>
                     <v-row>
                        ...
                        <!-- MODAL SELL-->
                        <v-dialog v-model="sellModal" max-width="400" dark>
                           <v-card>
                              <v-card-title class="text-h5 mb-5">
                                 Sell
                              </v-card-title>
                        ...
                        >
                           <div class="liked__card">
                              ...
                              >
                                 ...
                                 <p
                                    class="warning--text text-body-2 mt-1"
                                    v-else
                                 >
                                    Not for sale
                                 </p>
...
export default {
   ...
   methods: {
      ...
      async setNewPrice() {
         ...
         const price = ethers.utils.parseUnits(this.newPrice, "szabo");

         this.$store.state.confirm_msg = "Update Item Price";
         this.priceModal = false;
         this.$store.state.confirm = true;

         try {
            const transaction =
               await this.$store.state.marketContract.setMarketItemPrice(
                  this.newPriceItemId,
                  price
               );
            ...
            await this.$axios.$post(
               `/api/v1/items/update/`,
               { itemId: this.newPriceItemId },
               this.config
            );
            console.log(`Item #${this.newPriceItemId} price updated`);
            await this.loadData();
         } catch (error) {
      ...
      async unlist(id) {
         ...
         this.$store.state.confirm_msg = "Unlisting NFT";
         this.$store.state.confirm = true;
         try {
            const transaction =
               await this.$store.state.marketContract.unlistMarketItem(id);
            await transaction.wait();
            await this.$axios.$post(
               `/api/v1/items/update/`,
               { itemId: id },
      ...
      async confirmSell() {
         ...
         console.log("sell item", this.sellItemId);
         const price = ethers.utils.parseUnits(this.sellPrice, "szabo");
         this.$store.state.confirm_msg = "Place NFT on Market";
         this.sellModal = false;
         this.$store.state.confirm = true;
         try {
            if (!this.sellItemId) {
               this.sellItemId = await this.$daddy.createMarketItem(
                  this.tokenId,
                  this.sellPrice
               );
               await this.$axios.$post(
                  `/api/v1/items/update/`,
                  { id: this.nftId, itemId: this.sellItemId },
                  this.config
               );
            } else {
               const transaction =
                  await this.$store.state.marketContract.listMarketItem(
                     this.sellItemId,
                     price
                  );
               await transaction.wait();
               await this.$axios.$post(
                  `/api/v1/items/update/`,
                  { itemId: this.sellItemId },
                  this.config
               );
            }
            console.log(`NFT with itemId ${this.sellItemId} placed on market`);
            await this.loadData();
         } catch (error) {
   ...
   async mounted() {
      ...
      this.current_user = this.$store.state.currentWallet;
      await this.loadData();
   },
};
</script>
